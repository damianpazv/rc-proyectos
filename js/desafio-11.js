...
let mje = "hola"
    ...
    const busqueda=prompt("ingrese una o mas letras");

    const paisesFiltrados=[];

    paisesLatinoamericanos.forEach((pais) => {
        let nombremin=pais.nombre.toLowerCase();

        if(busqueda.length==1 && nombremin[0] == busqueda[0])
        {
        ...
        else if (busqueda.length>1 && nombremin.includes(busqueda) )
        {
            paisesFiltrados.push(pais);
        }
    ...
    paisesFiltrados.forEach((pais,indice) => {

        const position= String(indice + 1).padStart(2, '0');
